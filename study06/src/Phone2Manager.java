...
public class Phone2Manager {
    ...
    public void printPhone2Manager() {
        ...
        // 사용자가 입력한 인원수만큼 Phone2 클래스 타입의 객체를 생성해야 하므로 Phone2 클래스 타입의 배열을 선언
        ...
        for (int i = 0; i < count; i++) {
            ...
            // Phone2 클래스 타입의 객체를 count번 생성하고 배열에 저장함
            phone[i] = new Phone2(name, tel);

            if ((i + 1) == count) {
                System.out.println("저장되었습니다..");
                // Scanner를 통해서 키보드 입력을 받았을 경우 입력을 위해서 사용한 메서드의 종류에 따라 '\n'이 메모리에 남아있는 경우가 있음
                // nextLint()를 마지막으로 한 번 더 실행하여 버퍼의 내용을 완전히 비움
                sc.nextLine();
            }
        }
        ...
        while (true) {
            ...
            for (int i = 0; i < phone.length; i++) {
                // 입력받은 검색어와 동일한 내용이 있을 경우 검색 결과 출력용 변수에 데이터 입력 후 for 문 종료
